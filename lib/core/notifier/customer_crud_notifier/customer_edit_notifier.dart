...
import 'package:kenz_app/core/api/customer_crud_api/customer_edit_api.dart';
...
class CustomerEditNotifier extends ChangeNotifier {
  final CustomerEditCrudApi _customerApi = CustomerEditCrudApi();
  ...
  Future<String?> customerEdit({
    ...
    try {
      ...
      final listData = await _customerApi.customerEdit( name: name, nameA: nameA, address1: address1, address1A: address1A, address2: address2, address2A: address2A, crn: crn, vat: vat, isCustomer: isCustomer, isVendor: isVendor, isEmployee: isEmployee, isRep: isRep,userID: userID, customerID: customerID,    route: route,
        group: group,
        priceList: priceList,
        region: region,
        rep: rep,);
      ...
      if(listData["status"] == 200){
        ...
        showAwesomeDialogue(title: "Success", content:listData["message"] ?? "Customer Data Edited Successfully", type: DialogType.SUCCES,);
        notifyListeners();
        return "OK";
      }else{
        _isLoading = false;
